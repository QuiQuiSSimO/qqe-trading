...
.stApp { font-family:'Exo 2',sans-serif; color:#e8f2ff; font-size:17px; line-height:1.75; }
[data-testid="stSidebar"] { background:#080e1c !important; border-right:1px solid #1a2a45 !important; }

/* TABS - grandes, claros, legibles */
...
.stTabs [data-baseweb="tab"] {
    background:transparent; color:#90bcd8 !important;
    font-family:'Exo 2',sans-serif; font-weight:700;
    font-size:14px !important; letter-spacing:0.5px;
    padding:15px 20px; border:none; white-space:nowrap;
    transition:all 0.25s;
}
.stTabs [aria-selected="true"] {
    background:linear-gradient(180deg,#0d1e3a,#050a14) !important;
    color:#f5c842 !important; border-bottom:3px solid #f5c842 !important;
    font-weight:900 !important; font-size:14px !important;
    text-shadow:0 0 14px rgba(245,200,66,0.7);
}

/* CARDS */
.card { background:#0a1020; border:1px solid #1a2a45; border-radius:12px; padding:20px; margin-bottom:10px; }
...
/* KPIs - numeros grandes y labels claros */
...
.kpi-label { font-family:'Share Tech Mono',monospace; font-size:15px; color:#90bcd8;
             letter-spacing:2px; margin-bottom:8px; font-weight:700; text-transform:uppercase; }
.kpi-value { font-family:'Rajdhani',sans-serif; font-size:44px; font-weight:700; line-height:1.1; }
.kpi-sub   { font-family:'Share Tech Mono',monospace; font-size:15px; color:#90bcd8; margin-top:5px; }

/* SECCION HEADER */
.sec { font-family:'Share Tech Mono',monospace; font-size:13px; color:#f5c842;
       letter-spacing:3px; text-transform:uppercase; font-weight:700;
       border-bottom:1px solid #1a2a45; padding-bottom:10px; margin:20px 0 16px;
       display:flex; align-items:center; gap:10px; }
.sec::before { content:''; display:inline-block; width:4px; height:16px;
               background:linear-gradient(#f0b429,#c8920a); border-radius:2px; flex-shrink:0; }

/* SIGNALS */
.signal-call {
    background:linear-gradient(135deg,#001a08,#002010,#001508);
    border:1px solid #22c55e; border-radius:12px; padding:20px; margin-bottom:12px;
...
.signal-put {
    ...
    border:1px solid #ef4444; border-radius:12px; padding:20px; margin-bottom:12px;
    box-shadow:0 0 20px rgba(239,68,68,0.15), inset 0 1px 0 rgba(239,68,68,0.1);
    animation:glow-put 3s ease-in-out infinite;
}
.signal-wait { background:#0a1020; border:1px solid #1a2a45; border-radius:12px; padding:18px; margin-bottom:10px; }

@keyframes glow-call { 0%,100%{box-shadow:0 0 15px rgba(34,197,94,0.1);} 50%{box-shadow:0 0 35px rgba(34,197,94,0.35);} }
@keyframes glow-put  { 0%,100%{box-shadow:0 0 15px rgba(239,68,68,0.1);}  50%{box-shadow:0 0 35px rgba(239,68,68,0.35);} }
...
.btn-entrada {
    ...
    font-size:18px; letter-spacing:2px; padding:14px 24px; border-radius:10px;
    border:none; cursor:pointer; width:100%; text-align:center;
    box-shadow:0 4px 20px rgba(34,197,94,0.4); animation:pulse-btn 2s infinite;
}
.btn-entrada-put { background:linear-gradient(135deg,#ef4444,#dc2626) !important; box-shadow:0 4px 20px rgba(239,68,68,0.4) !important; }
@keyframes pulse-btn { 0%,100%{transform:scale(1);} 50%{transform:scale(1.02);} }

/* SESION BAR */
.sesion-bar {
    position:sticky; top:0; z-index:999; background:rgba(5,10,20,0.95); backdrop-filter:blur(12px);
    border-bottom:1px solid #1a2a45; padding:10px 16px;
    display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:8px;
    margin-bottom:16px; border-radius:0 0 10px 10px;
}

/* BADGES */
.badge { padding:5px 14px; border-radius:20px; font-family:'Share Tech Mono',monospace; font-size:15px; font-weight:700; }
.badge-green  { background:rgba(34,197,94,0.15);  color:#6ee7b7; border:1px solid #22c55e; }
.badge-red    { background:rgba(239,68,68,0.15);   color:#fca5a5; border:1px solid #ef4444; }
.badge-gold   { background:rgba(240,180,41,0.15);  color:#fde68a; border:1px solid #f0b429; }
.badge-blue   { background:rgba(59,130,246,0.15);  color:#93c5fd; border:1px solid #3b82f6; }
.badge-purple { background:rgba(168,85,247,0.15);  color:#d8b4fe; border:1px solid #a855f7; }
.badge-gray   { background:#1a2535; color:#cbd5e1; border:1px solid #2a3a55; }

/* CONTENIDO GENERAL */
.ia-box { background:#0a1020; border:1px solid #f0b429; border-radius:12px; padding:20px;
          font-size:16px; line-height:2.0; color:#e8f2ff; }
.asset-row { background:#0a1020; border:1px solid #1a2a45; border-radius:10px; padding:14px; margin-bottom:8px; transition:border-color 0.2s; }
...
.prog-track { height:10px; background:#1a2a45; border-radius:5px; overflow:hidden; margin:5px 0; }
.prog-fill  { height:100%; border-radius:5px; transition:width 0.8s ease; }
.diario-entry { background:#0a1020; border:1px solid #1a2a45; border-radius:10px; padding:16px; margin-bottom:8px; }
.diario-meta  { font-family:'Share Tech Mono',monospace; font-size:15px; color:#90bcd8; letter-spacing:1px; margin-bottom:6px; }
...
.code-block { background:#030710; border:1px solid #1a2a45; border-radius:8px; padding:16px;
              font-family:'Share Tech Mono',monospace; font-size:15px; line-height:2.0; color:#93d5ff;
              white-space:pre; overflow-x:auto; max-height:300px; overflow-y:auto; margin-bottom:10px; }
.strat-box  { background:#0a1020; border:1px solid #f0b429; border-radius:14px; padding:20px; margin-bottom:12px; }
.strat-step { background:#030710; border-left:3px solid #f0b429; padding:14px 18px; margin-bottom:8px;
              border-radius:0 10px 10px 0; font-size:15px; line-height:1.9; color:#e8f2ff; }
.strat-regla { background:rgba(34,197,94,0.05); border:1px solid #22c55e; border-radius:10px;
               padding:14px 18px; margin-bottom:8px; font-size:15px; color:#e8f2ff; }

/* SCROLLBAR */
::-webkit-scrollbar { width:5px; height:5px; }
::-webkit-scrollbar-track { background:#050a14; }
::-webkit-scrollbar-thumb { background:linear-gradient(#f0b429,#c8920a); border-radius:3px; }

/* BOTONES STREAMLIT */
.stButton>button {
    background:linear-gradient(135deg,#0d1830,#1a2a45) !important;
    color:#e8f2ff !important; border:1px solid #2a4060 !important;
    ...
    transition:all .25s; min-height:48px !important;
}
.stButton>button:hover {
    background:linear-gradient(135deg,#f0b429,#c8920a) !important;
    color:#000 !important; border-color:#f0b429 !important;
    box-shadow:0 4px 16px rgba(240,180,41,0.4) !important; transform:translateY(-1px);
}
.stButton>button:active { transform:translateY(0); }

/* INPUTS */
.stTextInput>div>div>input, .stTextArea>div>div>textarea {
    background:#0a1020 !important; border:1px solid #2a4060 !important;
    color:#e8f2ff !important; border-radius:10px !important; font-size:15px !important;
}
...
.stSlider label, .stRadio label, .stMultiSelect label {
    font-size:14px !important; color:#b0d0f0 !important;
    font-family:'Exo 2',sans-serif !important; font-weight:700 !important;
}
.stRadio > div { gap:14px; }
.stRadio label { font-size:14px !important; color:#e8f2ff !important; }

/* TEXTO GENERAL */
.stMarkdown p { font-size:15px; color:#e8f2ff; line-height:1.85; }
.stCaption, .stCaption p { font-size:13px !important; color:#90bcd8 !important; }
[data-testid="stDataFrame"] { font-size:13px !important; }

/* MOBILE */
@media (max-width:768px) {
    .stApp { font-size:15px !important; }
    .stTabs [data-baseweb="tab"] { font-size:15px !important; padding:12px 10px !important; }
    .stTabs [data-baseweb="tab-list"] { overflow-x:auto !important; -webkit-overflow-scrolling:touch; }
    .stButton>button { min-height:52px !important; font-size:14px !important; }
    .kpi-value { font-size:34px !important; }
    .kpi { padding:14px !important; }
}
@media (max-width:480px) {
    .stTabs [data-baseweb="tab"] { font-size:15px !important; padding:10px 8px !important; }
    .btn-entrada { font-size:17px !important; padding:16px !important; min-height:56px; }
    .kpi-value { font-size:28px !important; }
}

/* EXPANDERS - claros y grandes */
...
/* TEXTO BASE — mínimo 15px en todo */
p, div, li, span, td, th { font-size:15px !important; }
.stMarkdown, .stMarkdown p { font-size:15px !important; color:#e8f2ff !important; line-height:1.85 !important; }
.stCaption p { font-size:14px !important; color:#90bcd8 !important; }
small { font-size:14px !important; }

/* DATAFRAME - más legible */
[data-testid="stDataFrame"] th { font-size:13px !important; color:#f5c842 !important; background:#0d1830 !important; }
[data-testid="stDataFrame"] td { font-size:13px !important; color:#e8f2ff !important; }

/* SLIDER labels y valores */
[data-testid="stSlider"] p { font-size:15px !important; color:#e8f2ff !important; }
[data-testid="stSlider"] [data-testid="stMarkdownContainer"] p { font-size:13px !important; color:#90bcd8 !important; }

/* SELECTBOX options */
[data-baseweb="select"] * { font-size:15px !important; color:#e8f2ff !important; }
[data-baseweb="menu"] * { font-size:15px !important; }

/* RADIO */
.stRadio label span { font-size:15px !important; color:#e8f2ff !important; }

/* st.info / st.warning / st.error */
[data-testid="stAlert"] p { font-size:15px !important; }

/* Sub-tabs (tabs dentro de tabs) */
.stTabs .stTabs [data-baseweb="tab"] {
    font-size:13px !important;
    padding:11px 14px !important;
    color:#90bcd8 !important;
}
...
def _build_filtros_html(r):
    ...
    for i, lbl in enumerate(labels):
        ...
        parts.append(
            ...
            f'<div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">{lbl}</div>'
...
def _kpi_v11(label, value, sub="", color="#c8d8e8"):
    st.markdown(f"""<div style="background:linear-gradient(135deg,#0a1020,#0d1830);border:1px solid #1a2a45;
        ...
        <div style="font-family:'Share Tech Mono',monospace;font-size:14px;color:#a0c8e8;letter-spacing:2px;margin-bottom:6px;">{label}</div>
        <div style="font-family:'Rajdhani',sans-serif;font-size:34px;font-weight:700;color:{color};line-height:1.1;">{value}</div>
        {"<div style='font-family:Share Tech Mono,monospace;font-size:15px;color:#a0c8e8;margin-top:3px;'>"+sub+"</div>" if sub else ""}
...
with st.sidebar:
    ...
    # API
    st.markdown('<div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#a0c8e8;letter-spacing:2px;margin-bottom:4px;">CLAVE API ANTHROPIC</div>', unsafe_allow_html=True)
    ...
    st.markdown(f'<div style="font-family:Share Tech Mono,monospace;font-size:15px;color:{ia_col};margin-bottom:12px;">{ia_status}</div>', unsafe_allow_html=True)

    st.markdown("---")

    # Capital
    st.markdown('<div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#a0c8e8;letter-spacing:2px;margin-bottom:4px;">CAPITAL Y RIESGO</div>', unsafe_allow_html=True)
    ...
    <div style="background:#0d1525;border:1px solid #1e3050;border-radius:8px;padding:12px;margin-bottom:8px;">
      <div style="display:flex;justify-content:space-between;margin-bottom:4px;">
        <span style="font-family:Share Tech Mono,monospace;font-size:15px;color:#a0c8e8;">ENTRADA 1%</span>
        <span style="font-family:Rajdhani,sans-serif;font-weight:700;font-size:17px;color:#4ade80;">${cap*0.01:.2f}</span>
      </div>
      <div style="display:flex;justify-content:space-between;margin-bottom:8px;">
        <span style="font-family:Share Tech Mono,monospace;font-size:15px;color:#a0c8e8;">ENTRADA 2%</span>
      ...
      <div style="font-family:Share Tech Mono,monospace;font-size:15px;color:{bar_col};margin-top:3px;">Pérdida: ${perdida:.2f} / SL: ${sl_monto:.2f}</div>
      <div style="display:flex;justify-content:space-between;margin-top:8px;padding-top:8px;border-top:1px solid #1e3050;">
        <span style="font-family:Share Tech Mono,monospace;font-size:15px;color:#a0c8e8;">RACHA LOSSES</span>
    ...
    # Telegram
    st.markdown('<div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#a0c8e8;letter-spacing:2px;margin-bottom:4px;">TELEGRAM</div>', unsafe_allow_html=True)
    ...
    if tg_chk:
        ...
        st.markdown(f'<div style="font-family:Share Tech Mono,monospace;font-size:15px;color:{col_tg};margin-bottom:6px;">{"CONECTADO ✓" if tg_ok else "Completa token y chat ID"}</div>', unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.markdown('<div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#7090b0;text-align:center;">Credenciales guardadas automáticamente</div>', unsafe_allow_html=True)
...
<div style="background:linear-gradient(135deg,#080e1c,#0d1830);border:1px solid #1a2a45;border-radius:14px;
  ...
  <div>
    ...
    <div style="font-family:'Share Tech Mono',monospace;font-size:14px;color:#a0c8e8;letter-spacing:2px;">
  ...
  <div style="background:{sem_bg};border:1px solid {sem_color};border-radius:10px;padding:8px 14px;text-align:center;min-width:120px;">
    <div style="font-family:'Share Tech Mono',monospace;font-size:14px;color:{sem_color};letter-spacing:2px;">{sem_icon} SESION</div>
    <div style="font-family:'Rajdhani',sans-serif;font-weight:700;font-size:14px;color:{sem_color};">{ses_nom}</div>
    <div style="font-family:'Share Tech Mono',monospace;font-size:14px;color:{sem_color};opacity:0.8;">{ses_est}</div>
  </div>
  <div style="text-align:center;">
    <div style="font-family:'Share Tech Mono',monospace;font-size:14px;color:#a0c8e8;letter-spacing:2px;">🕐 ARG</div>
  ...
  <div style="text-align:center;">
    <div style="font-family:'Share Tech Mono',monospace;font-size:14px;color:#a0c8e8;letter-spacing:2px;">P&L HOY</div>
    <div style="font-family:'Rajdhani',sans-serif;font-weight:700;font-size:28px;color:{pnl_col};">${{np_:+.2f}}</div>
  </div>
  <div style="text-align:center;">
    <div style="font-family:'Share Tech Mono',monospace;font-size:14px;color:#a0c8e8;letter-spacing:2px;">RACHA</div>
    <div style="font-family:'Rajdhani',sans-serif;font-weight:700;font-size:26px;color:{racha_col};">{_racha_hdr}</div>
  </div>
  <div style="text-align:center;">
    <div style="font-family:'Share Tech Mono',monospace;font-size:14px;color:#a0c8e8;letter-spacing:2px;">ENTRADA 1%</div>
...
with tab_scan1:
    ...
    with c3:
        if ar_on:
            ...
            st.markdown(f'<div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#4ade80;">● ACTIVO — {ar_min}min</div>', unsafe_allow_html=True)
    with c4:
    ...
    if st.session_state.radar_bloqueado:
        ...
        for idx, activo in enumerate(activos_sel1):
            ...
            txt.markdown(f'<div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#c8920a;">⏳ Escaneando {activo}... ({idx+1}/{len(activos_sel1)})</div>', unsafe_allow_html=True)
    ...
    if st.session_state.scan_results:
        if st.session_state.scan_ultimo:
            ...
            st.markdown(f'<div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;margin-bottom:12px;"><span class="scan-dot" style="background:#4ade80;"></span> Ultimo scan: {st.session_state.scan_ultimo} — <span style="color:{"#4ade80" if n_al>0 else "#4a7a99"};">{n_al} señales activas</span></div>', unsafe_allow_html=True)
            ...
            <div class="{card_cls}">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;flex-wrap:wrap;gap:6px;">
                <div>
                  ...
                  <span style="font-family:Share Tech Mono,monospace;font-size:14px;color:{col_dir};margin-left:10px;">{ic} {m['dir']} — Script {m['script']}</span>
                </div>
                <div style="text-align:right;">
                  <div style="font-family:Rajdhani,sans-serif;font-weight:700;font-size:22px;color:{col_dir};">{conf}%</div>
                  <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">CONFIANZA</div>
                </div>
              </div>
              {conf_bar}
              <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:10px;">
                <div style="background:#060c18;border-radius:6px;padding:8px;text-align:center;">
                  <div style="font-family:Share Tech Mono,monospace;font-size:8px;color:#a0c8e8;">PRECIO</div>
                  <div style="font-family:Rajdhani,sans-serif;font-weight:700;font-size:14px;color:#c8d8e8;">{r['precio']:.4f}</div>
                </div>
                <div style="background:#060c18;border-radius:6px;padding:8px;text-align:center;">
                  <div style="font-family:Share Tech Mono,monospace;font-size:8px;color:#a0c8e8;">RSI</div>
                  <div style="font-family:Rajdhani,sans-serif;font-weight:700;font-size:14px;color:{_rsi_col};">{r['rsi']:.1f}</div>
                </div>
                <div style="background:#060c18;border-radius:6px;padding:8px;text-align:center;">
                  <div style="font-family:Share Tech Mono,monospace;font-size:8px;color:#a0c8e8;">ENTRADA 1%</div>
                ...
                <div style="background:#060c18;border-radius:6px;padding:8px;text-align:center;">
                  <div style="font-family:Share Tech Mono,monospace;font-size:8px;color:#a0c8e8;">EXPIRACION</div>
                  <div style="font-family:Rajdhani,sans-serif;font-weight:700;font-size:14px;color:#c8d8e8;">1-2 min</div>
                </div>
              </div>
              {"<br>".join([f'<span style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">• Script {s["script"]}: {s["dir"]} {s["conf"]}%</span>' for s in r["senales"]])}
            </div>""", unsafe_allow_html=True)
    else:
        st.markdown("""
        <div style="text-align:center;padding:60px 20px;background:#0d1525;border:2px dashed #1e3050;border-radius:12px;margin-top:10px;">
          ...
          <div style="font-family:Rajdhani,sans-serif;font-size:20px;color:#a0c8e8;">Presiona ESCANEAR AHORA</div>
          <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#7090b0;margin-top:6px;">Detecta señales de binarias 1 minuto en tiempo real</div>
        </div>""", unsafe_allow_html=True)

    # Advertencia 1min
    st.markdown("""
    <div style="background:#2a1a00;border:1px solid #c8920a;border-radius:8px;padding:10px 14px;margin-top:12px;font-size:15px;color:#fbbf24;">
...
with tab_swing:
    ...
    <div style="background:#0d1525;border:2px solid {ses_col};border-radius:12px;padding:12px 18px;margin-bottom:14px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
      <div>
        <span style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;letter-spacing:2px;">SESION ACTIVA</span>
        <div style="font-family:Rajdhani,sans-serif;font-weight:700;font-size:22px;color:{ses_col};">🌐 {sesion_activa}</div>
        <div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#c0d8f0;">{ses_desc}</div>
      </div>
      <div style="text-align:right;">
        <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">TEMPORALIDAD OPTIMA</div>
        <div style="font-family:Rajdhani,sans-serif;font-weight:700;font-size:28px;color:{ses_col};">{tf_recom}</div>
        <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">{hora_arg_disp} ARG · {hora_utc:02d}:xx UTC</div>
      </div>
    </div>""", unsafe_allow_html=True)
    ...
    <div style="background:{badge[2]};border:1px solid {badge[1]};border-radius:8px;padding:8px 14px;margin-bottom:10px;display:flex;justify-content:space-between;align-items:center;">
      <div>
        ...
        <span style="font-family:Share Tech Mono,monospace;font-size:15px;color:#c0d8f0;margin-left:12px;">{badge[3]}</span>
      </div>
      <span style="font-family:Share Tech Mono,monospace;font-size:14px;color:{badge[1]};border:1px solid {badge[1]};border-radius:4px;padding:2px 8px;">{badge[4]}</span>
    </div>""", unsafe_allow_html=True)
    ...
    if swing_btn:
        ...
        for idx, activo in enumerate(activos_swing):
            ...
            txt2.markdown(f'<div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#c8920a;">📈 Analizando {activo} {tf_sel}... ({idx+1}/{len(activos_swing)})</div>', unsafe_allow_html=True)
    ...
    if hasattr(st.session_state, "swing_resultados") and st.session_state.swing_resultados:
        ...
        if top_signals:
            st.markdown('<div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#c8920a;letter-spacing:2px;margin:8px 0 6px;">SEÑALES ACTIVAS</div>', unsafe_allow_html=True)
            ...
            for i, r in enumerate(top_signals[:3]):
                ...
                with cols_sig[i]:
                    ...
                    <div style="background:{c_bg};border:2px solid {c_bo};border-radius:14px;padding:16px;text-align:center;">
                      <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;letter-spacing:2px;">{tipo_lbl} · {r["tf"]}</div>
                      ...
                      <div style="font-family:Share Tech Mono,monospace;font-size:15px;color:{c_tx};">AHORA · {r["hora"]}</div>
                      <div style="margin-top:10px;display:grid;grid-template-columns:1fr 1fr;gap:6px;">
                        <div style="background:#060c18;border-radius:6px;padding:6px;">
                          <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">SCORE</div>
                          <div style="font-family:Rajdhani,sans-serif;font-weight:700;font-size:20px;color:{c_tx};">{r["score"]}%</div>
                        </div>
                        <div style="background:#060c18;border-radius:6px;padding:6px;">
                          <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">ENTRADA</div>
                          <div style="font-family:Rajdhani,sans-serif;font-weight:700;font-size:14px;color:#c8d8e8;">{r["precio"]:.4f}</div>
                        </div>
                      </div>
        ...
        # Lista completa
        st.markdown('<div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;letter-spacing:2px;margin:4px 0 8px;">TODOS LOS ACTIVOS ESCANEADOS</div>', unsafe_allow_html=True)
        for r in res_list:
            es_long  = "LONG" in r["direccion"]
            ...
            <div class="{card_cls}">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;flex-wrap:wrap;gap:6px;">
                <div>
                  ...
                  <span style="font-family:Share Tech Mono,monospace;font-size:14px;color:{tipo_col};margin-left:8px;">{tipo_lbl}</span>
                  <span style="font-family:Share Tech Mono,monospace;font-size:15px;color:{col_dir};margin-left:8px;">{ic} {r["direccion"]}</span>
                  <span style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;margin-left:8px;">{r.get("tf","")}</span>
                </div>
                <div style="text-align:right;">
              ...
              <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;">
                <div style="background:#060c18;border-radius:6px;padding:10px;text-align:center;">
                  <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">PRECIO</div>
                ...
                <div style="background:#3a0a0a;border:1px solid #dc2626;border-radius:6px;padding:10px;text-align:center;">
                  <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#f87171;">STOP LOSS</div>
                ...
                <div style="background:#0a3020;border:1px solid #16a34a;border-radius:6px;padding:10px;text-align:center;">
                  <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#4ade80;">TAKE PROFIT</div>
                  <div style="font-family:Rajdhani,sans-serif;font-weight:700;font-size:15px;color:#4ade80;">{r["tp"]:.5f}</div>
                </div>
              </div>
              <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-top:6px;">
                <div style="text-align:center;"><span style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">RSI: </span><span style="font-family:Rajdhani,sans-serif;font-weight:700;font-size:13px;color:#c8d8e8;">{r["rsi"]:.1f}</span></div>
                <div style="text-align:center;"><span style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">EMA20: </span><span style="font-family:Rajdhani,sans-serif;font-weight:700;font-size:13px;color:#c8d8e8;">{r["ema20"]:.5f}</span></div>
                <div style="text-align:center;"><span style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">ATR: </span><span style="font-family:Rajdhani,sans-serif;font-weight:700;font-size:13px;color:#c8d8e8;">{r["atr"]:.5f}</span></div>
              </div>
            </div>""", unsafe_allow_html=True)
    else:
        st.markdown("""
        <div style="text-align:center;padding:60px 20px;background:#0d1525;border:2px dashed #1e3050;border-radius:12px;margin-top:10px;">
          <div style="font-size:48px;margin-bottom:10px;">📈</div>
          <div style="font-family:Rajdhani,sans-serif;font-size:20px;color:#a0c8e8;">Presiona ESCANEAR SWING</div>
          <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#7090b0;margin-top:6px;">Analiza M15/H1/H4/D1 con SL y TP calculados por ATR</div>
        </div>""", unsafe_allow_html=True)
    ...
    <div style="background:#0d1525;border:1px solid #c8920a;border-radius:12px;padding:12px 18px;margin-bottom:14px;">
      ...
      <div style="font-size:13px;color:#c0d8f0;line-height:1.9;">
    ...
    if not get_ia_ok():
        st.markdown('<div style="background:#3a0a0a;border:1px solid #dc2626;border-radius:8px;padding:12px;color:#f87171;font-family:Share Tech Mono,monospace;font-size:15px;">⚠ Necesitas la clave API de Anthropic en el sidebar para usar el analizador.</div>', unsafe_allow_html=True)
    else:
        ...
        st.markdown(f'<div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#c8920a;margin:4px 0;">📋 Nombre en IQ Option: <b>{iqopt_name}</b></div>', unsafe_allow_html=True)

        st.markdown('<div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;letter-spacing:2px;margin:8px 0 4px;">CAPTURA DEL GRAFICO (hasta 3) · 📱 iPhone: capturá → Archivos → subí</div>', unsafe_allow_html=True)
        ...
        if imgs:
            ...
            for i, img in enumerate(imgs[:3]):
                with cols_prev[i]:
                    ...
                    st.markdown(f'<div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;text-align:center;">{img.name}</div>', unsafe_allow_html=True)
...
Si el grafico es poco claro, decilo sin dudar."""
            ...
            <div style="text-align:center;padding:40px 20px;background:#0d1525;border:2px dashed #1e3050;border-radius:12px;margin-top:10px;">
              <div style="font-size:48px;margin-bottom:10px;">📱</div>
              <div style="font-family:Rajdhani,sans-serif;font-size:18px;color:#a0c8e8;">Subi la captura de IQ Option</div>
              <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#7090b0;margin-top:6px;">Hasta 3 graficos — M15 + H1 + H4</div>
            </div>""", unsafe_allow_html=True)
        ...
        if resp_key in st.session_state and st.session_state[resp_key]:
            ...
            <div style="background:{rb};border:2px solid {rbr};border-radius:14px;padding:20px;margin-top:16px;">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
                ...
                <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">{res_ia["tf"]} · {res_ia["hora"]}</div>
              </div>
              <div style="font-size:14px;color:#c8d8e8;line-height:2.1;">{texto.replace(chr(10),"<br>")}</div>
              <div style="margin-top:12px;padding-top:10px;border-top:1px solid {rbr};font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">
...
with tab_triple:
    ...
    <div style="background:linear-gradient(135deg,#0d1525 0%,#0a1e0a 100%);border:1px solid #c8920a;border-radius:14px;padding:18px 22px;margin-bottom:18px;">
      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;">
        <div>
          ...
          <div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#a0c8e8;margin-top:4px;">4 FILTROS · HECTOR 1 · EMA + MACD + RSI + VELA</div>
        </div>
        <div style="display:flex;gap:16px;flex-wrap:wrap;">
          <div style="text-align:center;background:#060c18;border:1px solid #16a34a;border-radius:8px;padding:10px 18px;">
            ...
            <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">= ENTRAR</div>
          </div>
          <div style="text-align:center;background:#060c18;border:1px solid #ff9800;border-radius:8px;padding:10px 18px;">
            ...
            <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">= ESPERAR</div>
          </div>
          <div style="text-align:center;background:#060c18;border:1px solid #dc2626;border-radius:8px;padding:10px 18px;">
            ...
            <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">= NO OPERAR</div>
          </div>
        </div>
      </div>
    </div>
    ...
    with st.expander("📖 Ver reglas de los 4 filtros", expanded=False):
        ...
        with col_rc:
            ...
            <div style="background:#0a2010;border:1px solid #16a34a;border-radius:10px;padding:14px;">
              ...
              <div style="font-size:13px;color:#c0d8f0;line-height:2.1;">
        ...
        with col_rp:
            ...
            """, unsafe_allow_html=True)
        st.markdown("""
        <div style="background:#0d1525;border:1px solid #1e3050;border-radius:8px;padding:12px;margin-top:10px;font-size:13px;color:#c0d8f0;line-height:1.8;">
    ...
    if triple_btn:
        ...
        for idx, activo in enumerate(activos_triple):
            ...
            txt_t.markdown(f'<div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#c8920a;">⏳ Escaneando {activo}... ({idx+1}/{len(activos_triple)})</div>', unsafe_allow_html=True)
    ...
    if st.session_state.get("triple_results") is not None:
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:10px;margin-bottom:16px;">
          <div class="kpi"><div class="kpi-label">ÚLTIMO SCAN</div><div class="kpi-value" style="font-size:20px;color:#a0c8e8;">{ultimo_t}</div></div>
        ...
        if completos_t:
            st.markdown('<div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#4ade80;letter-spacing:2px;margin-bottom:8px;">⚡ SEÑALES COMPLETAS — ENTRAR AHORA</div>', unsafe_allow_html=True)
            for r in completos_t:
                ...
                if alineado and es_horario_operable():
                    calidad_label = '<span style="background:#16a34a;color:#fff;padding:3px 10px;border-radius:6px;font-family:Share Tech Mono,monospace;font-size:14px;margin-left:8px;">⭐ ALTA CALIDAD</span>'
                elif contra:
                    calidad_label = '<span style="background:#c8920a;color:#000;padding:3px 10px;border-radius:6px;font-family:Share Tech Mono,monospace;font-size:14px;margin-left:8px;">⚠ CONTRA H1</span>'
                else:
                    calidad_label = ''
                ...
                if alineado and not contra:
                    _banner = '<div style="background:#0a3020;border:1px solid #16a34a;border-radius:6px;padding:8px;margin-top:8px;font-family:Share Tech Mono,monospace;font-size:15px;color:#4ade80;">&#9889; SEÑAL COMPLETA + H1 ALINEADO — Maxima calidad.</div>'
                elif contra:
                    _banner = '<div style="background:#2a1a00;border:1px solid #ff9800;border-radius:6px;padding:8px;margin-top:8px;font-family:Share Tech Mono,monospace;font-size:15px;color:#fbbf24;">&#9888; Señal OK pero contra H1. Reducir monto a 0.5%.</div>'
                else:
                    _banner = '<div style="background:#0a2030;border:1px solid #60a5fa;border-radius:6px;padding:8px;margin-top:8px;font-family:Share Tech Mono,monospace;font-size:15px;color:#60a5fa;">Señal completa. H1 lateral.</div>'
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;margin-bottom:14px;">
    ...
    <span style="font-family:Share Tech Mono,monospace;font-size:15px;color:{tipo_col};margin-left:12px;">{tipo_lbl}</span>
    {calidad_label}</div>
    <div style="display:flex;align-items:center;gap:16px;">
      <div style="text-align:center;">
        ...
        <div style="font-family:Share Tech Mono,monospace;font-size:15px;color:{col_d};">{_txt_ent}</div>
      </div>
      <div style="text-align:center;background:#060c18;border:1px solid {borde_col};border-radius:10px;padding:10px 16px;">
        ...
        <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">FILTROS</div>
      </div>
    </div>
  </div>
  ...
  <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:8px;">
    ...
    <div style="background:#060c18;border-radius:6px;padding:8px;text-align:center;">
      <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">RSI</div>
    ...
    <div style="background:#060c18;border:1px solid {_h1_border};border-radius:6px;padding:8px;text-align:center;">
      <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">H1 TREND</div>
    ...
    <div style="background:#060c18;border-radius:6px;padding:8px;text-align:center;">
      <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">ENTRADA 1%</div>
  ...
  {_banner}
                ...
                <div style="margin-top:12px;">
                  <div style="font-family:'Share Tech Mono',monospace;font-size:14px;color:#a0c8e8;
        ...
        if avisos_t:
            st.markdown('<div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#ff9800;letter-spacing:2px;margin:14px 0 8px;">⚠ PREPARARSE — 3/4 FILTROS</div>', unsafe_allow_html=True)
        ...
        <div style="text-align:center;padding:60px 20px;background:#0d1525;border:2px dashed #1e3050;border-radius:14px;margin-top:10px;">
          ...
          <div style="font-family:Rajdhani,sans-serif;font-size:22px;color:#a0c8e8;">Presioná ESCANEAR AHORA</div>
          <div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#7090b0;margin-top:6px;">4 filtros · todos los activos · señal clara en segundos</div>
        </div>""", unsafe_allow_html=True)
...
with tab_noticias:
    ...
    <div style="background:linear-gradient(135deg,#0d1525 0%,#1a0d25 100%);border:1px solid #7c3aed;border-radius:14px;padding:18px 22px;margin-bottom:18px;">
      ...
      <div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#a0c8e8;margin-top:4px;">
    ...
    with st.expander("📸 Pegar imagen del gráfico (portapapeles / captura)", expanded=False):
        ...
        <div style="background:rgba(168,85,247,0.08);border:1px solid #a855f7;border-radius:10px;
             padding:12px 16px;margin-bottom:10px;font-family:'Share Tech Mono',monospace;font-size:15px;color:#c084fc;">
    ...
    with st.expander("📅 PRÓXIMOS EVENTOS DE ALTO IMPACTO", expanded=True):
     pass
    ...
    for ev in calendario_eventos:
        ...
        <div style="background:#0d1525;border-left:4px solid {ev['color']};border-radius:0 8px 8px 0;padding:10px 14px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:6px;">
          <div>
            ...
            <span style="font-family:Share Tech Mono,monospace;font-size:14px;color:{ev['color']};margin-left:10px;">{ev['impacto']}</span>
          </div>
          <div style="text-align:right;">
            <div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#a0c8e8;">{ev['dia']}</div>
            <div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#60a5fa;">{ev['activos']}</div>
          </div>
        </div>"""
    cal_html += "</div>"
    ...
    st.markdown("""
    <div style="background:#1a0a0a;border:1px solid #dc2626;border-radius:8px;padding:10px 14px;margin:10px 0;font-family:Share Tech Mono,monospace;font-size:15px;color:#f87171;">
...
"""
    ...
    if st.session_state.get("noticias_analisis"):
        ...
        <div style="background:{senal_bg};border:2px solid {senal_color};border-radius:14px;padding:22px;margin-bottom:16px;">
          <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;margin-bottom:16px;">
            <div>
              ...
              <div style="font-family:Share Tech Mono,monospace;font-size:15px;color:{tend_color};margin-top:4px;">TENDENCIA: {an.get('tendencia','')}</div>
            </div>
            <div style="display:flex;gap:14px;align-items:center;flex-wrap:wrap;">
              <div style="text-align:center;background:#060c18;border:2px solid {senal_color};border-radius:12px;padding:14px 24px;">
                ...
                <div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#a0c8e8;">SEÑAL IA</div>
              </div>
          ...
          <div style="margin-bottom:14px;">
            <div style="display:flex;justify-content:space-between;margin-bottom:6px;">
              <span style="font-family:Share Tech Mono,monospace;font-size:15px;color:#4ade80;">🔺 SUBE {prob_suba}%</span>
              <span style="font-family:Share Tech Mono,monospace;font-size:15px;color:#f87171;">🔻 BAJA {prob_baja}%</span>
            </div>
          ...
          <div style="font-size:14px;color:#c0d8f0;line-height:1.7;border-top:1px solid #1e3050;padding-top:12px;">{an.get('resumen','')}</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if noticias_list:
            st.markdown('<div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#7c3aed;letter-spacing:2px;margin-bottom:8px;">📰 NOTICIAS ANALIZADAS</div>', unsafe_allow_html=True)
                ...
                <div style="background:{imp_bg};border:1px solid {imp_col};border-radius:10px;padding:12px 16px;margin-bottom:8px;">
                  <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:6px;">
                    <div style="flex:1;">
                      ...
                      <span style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;margin-left:10px;">{noticia.get('fuente','')}</span>
                    </div>
                    <span style="font-family:Rajdhani,sans-serif;font-weight:700;font-size:14px;color:{imp_col};">{imp_ic} {imp}</span>
                  </div>
                  <div style="font-size:13px;color:#c0d8f0;margin-top:6px;line-height:1.6;">{noticia.get('descripcion','')}</div>
                </div>""", unsafe_allow_html=True)
        ...
        with col_d1:
            ...
            if drivers:
                ...
                <div style="background:#0d1525;border:1px solid #7c3aed;border-radius:10px;padding:14px;">
                  <div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#7c3aed;margin-bottom:8px;letter-spacing:1px;">🎯 DRIVERS CLAVE</div>
        ...
        with col_d2:
            ...
            <div style="background:#0d1525;border:1px solid {riesgo_color};border-radius:10px;padding:14px;">
              <div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#a0c8e8;margin-bottom:8px;letter-spacing:1px;">⚠ RIESGO &amp; INVALIDACIÓN</div>
              ...
              <div style="font-size:13px;color:#c0d8f0;line-height:1.6;">{an.get('invalidacion','')}</div>
              <div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#a0c8e8;margin-top:8px;">Horizonte: {an.get('horizonte','')}</div>
            </div>""", unsafe_allow_html=True)
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-top:16px;">
          <div style="background:#0d1525;border:1px solid #fbbf24;border-radius:12px;padding:18px;text-align:center;">
            ...
            <div style="font-size:15px;color:#c0d8f0;margin-top:6px;line-height:1.6;">
        ...
        <div style="text-align:center;padding:24px;margin-top:10px;">
          <div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#a0c8e8;">Seleccioná un activo y hacé clic en 🤖 ANALIZAR CON IA</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        for o in reversed(st.session_state.ops[-20:]):
            ...
            <div class="asset-row" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:6px;">
              <div>
                ...
                <span style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;margin-left:8px;">{o['tipo']}</span>
                <span style="font-family:Share Tech Mono,monospace;font-size:14px;color:{_col_dir};margin-left:8px;">{ic_d} {o['dir']}</span>
              </div>
              <div style="display:flex;gap:14px;align-items:center;">
                <span style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">{o['fecha']} {o['hora']}</span>
...
with tab_diario:
    ...
    if st.session_state.diario:
        ...
        st.markdown("---")
        st.markdown('<div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#c8920a;letter-spacing:2px;margin-bottom:8px;">EXPORTAR BITACORA</div>', unsafe_allow_html=True)
        ...
        # ── ENTRADAS ─────────────────────────────────────────────
        st.markdown(f'<div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;letter-spacing:2px;margin-bottom:8px;">REGISTROS ({len(st.session_state.diario)} total)</div>', unsafe_allow_html=True)
        ...
        st.markdown('<div style="text-align:center;padding:40px;color:#a0c8e8;font-family:Share Tech Mono,monospace;font-size:15px;">Sin entradas aun. Guardá tu primera nota arriba.</div>', unsafe_allow_html=True)
...
with tab_binarias:
    ...
    st.info("Hacé clic en cada script para ver el código completo.")

    st.markdown(f"""
    <div style="background:#2a1a00;border:1px solid #c8920a;border-radius:8px;padding:10px 14px;margin-bottom:14px;font-size:15px;color:#fbbf24;">
...
function update()
    ...
    with s_a:
        ...
        <div style="font-size:13px;color:#c0d8f0;line-height:1.7;">
    ...
    <div style="background:#0d1525;border:1px solid #c8920a;border-radius:12px;padding:16px;margin-bottom:16px;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
        <div style="background:#0a3020;border:1px solid #16a34a;border-radius:8px;padding:12px;text-align:center;">
          <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#4ade80;">TRADING PROPIO (70%)</div>
          ...
          <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">QQE Command v4</div>
        </div>
        <div style="background:#0a1a3a;border:1px solid #2563eb;border-radius:8px;padding:12px;text-align:center;">
          <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#60a5fa;">COPY TRADING (30%)</div>
          ...
          <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">1 trader riesgo BAJO</div>
        </div>
      </div>
    </div>""", unsafe_allow_html=True)
    ...
    for name, wr, mult, risk, col, bg, border, desc in traders:
        ...
        <div style="background:{bg};border:1px solid {border};border-radius:10px;padding:14px;margin-bottom:8px;">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:8px;">
            <div style="background:#060c18;border-radius:6px;padding:8px;text-align:center;">
              <div style="font-family:Share Tech Mono,monospace;font-size:8px;color:#a0c8e8;">WIN RATE</div>
              <div style="font-family:Rajdhani,sans-serif;font-weight:700;font-size:18px;color:{col};">{wr}</div>
            </div>
            <div style="background:#060c18;border-radius:6px;padding:8px;text-align:center;">
              <div style="font-family:Share Tech Mono,monospace;font-size:8px;color:#a0c8e8;">MULTIPLICADOR</div>
              <div style="font-family:Rajdhani,sans-serif;font-weight:700;font-size:18px;color:{col};">{mult}</div>
            </div>
            <div style="background:#060c18;border-radius:6px;padding:8px;text-align:center;">
              <div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;">COPIA 30%</div>
          ...
          <div style="font-size:15px;color:#c0d8f0;line-height:1.6;">{desc}</div>
        </div>""", unsafe_allow_html=True)
...
# ================================================================
with tab_codigos:
    with st.expander("💻 CÓDIGOS QQE 03/04 — COPIAR EN IQ OPTION", expanded=False):
     st.markdown('Ver código completo abajo')
...
function update()
    ...
    with col_c1:
        ...
        st.markdown('<div style="font-size:15px;color:#c0d8f0;margin-bottom:8px;">EMA100 + EMA200 + MACD + Estocastico + RSI<br>Activos: XAU/USD, Crude Oil, EUR/USD, NAS100</div>', unsafe_allow_html=True)
    ...
    with col_c2:
        ...
        st.markdown('<div style="font-size:15px;color:#c0d8f0;margin-bottom:8px;">EMA50 + EMA200 + MACD + RSI<br>Activos: BTC, ETH, SOL</div>', unsafe_allow_html=True)
...
with tab_backtest:
    ...
    with bt_c1:
        ...
        st.markdown('<div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#f0b429;letter-spacing:2px;margin:12px 0 8px;">PARÁMETROS QQE</div>', unsafe_allow_html=True)
    ...
    with bt_c2:
        ...
        if "bt_result_v11" in st.session_state:
            ...
            if _s:
                ...
                with _bc1:
                    st.markdown('<div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;margin:12px 0 4px;">WIN RATE</div>', unsafe_allow_html=True)
                    _progbar(_s["win_rate"], "#22c55e")
                    st.markdown('<div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;margin:8px 0 4px;">PROFIT FACTOR (escala x10)</div>', unsafe_allow_html=True)
                    _progbar(min(100, _s["profit_factor"]*10), "#f0b429")
                with _bc2:
                    _tot_wr = (_s["avg_win"] + _s["avg_loss"]) or 1
                    st.markdown('<div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;margin:12px 0 4px;">WIN PROM vs LOSS PROM</div>', unsafe_allow_html=True)
            ...
            st.markdown("""<div style="text-align:center;padding:60px;background:#0a1020;border:2px dashed #1a2a45;border-radius:14px;">
              ...
              <div style="font-family:Rajdhani,sans-serif;font-size:22px;color:#a0c8e8;">Ajustá los parámetros y presioná ▶ CORRER BACKTEST</div>
              <div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#7090b0;margin-top:6px;">Win Rate · Profit Factor · Drawdown · RR Ratio · Curva de capital</div>
            </div>""", unsafe_allow_html=True)
    ...
    with sim_c2:
        ...
        if "sim_result_v11" in st.session_state:
            ...
            if not _trades2:
                ...
                with st.expander("📋 OPERACIONES DE LA SIMULACIÓN", expanded=False):
                 for _t2 in reversed(_trades2):
                    ...
                    st.markdown(f"""<div style="display:flex;justify-content:space-between;align-items:center;
                      ...
                      <div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#a0c8e8;">{_t2['fecha']}</div>
                      <div style="font-family:Rajdhani,sans-serif;font-size:16px;font-weight:700;color:{_ct2};">{_it2} {_t2['senal']}</div>
                      <div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#c8d8e8;">RSI {_t2['rsi']}</div>
                      <div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#c8d8e8;">{_t2['entrada']:.5f} → {_t2['salida']:.5f}</div>
                      <div style="font-family:Rajdhani,sans-serif;font-size:18px;font-weight:700;color:{_ct2};">{_t2['ganancia']:+.2f}$</div>
                      <div style="font-family:Share Tech Mono,monospace;font-size:15px;font-weight:700;
                ...
                with st.expander("📈 CURVA DE CAPITAL SIMULADA", expanded=True):
                    st.line_chart(pd.DataFrame({"Capital": _cap_c2}), color="#f0b429")
        else:
            st.markdown("""<div style="text-align:center;padding:60px;background:#0a1020;border:2px dashed #1a2a45;border-radius:14px;">
              <div style="font-size:48px;margin-bottom:12px;">🔮</div>
              <div style="font-family:Rajdhani,sans-serif;font-size:22px;color:#a0c8e8;">Configurá el escenario y presioná SIMULAR</div>
            </div>""", unsafe_allow_html=True)
...
with tab_educativo:
    ...
    with edu_t[0]:
        st.markdown("""<div style="background:#0a1020;border:1px solid #1a2a45;border-left:4px solid #f0b429;border-radius:12px;padding:18px;margin-bottom:12px;">
          <div style="font-family:'Share Tech Mono',monospace;font-size:15px;color:#f0b429;letter-spacing:2px;margin-bottom:12px;">QUE ES EL QQE</div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
          <div style="background:#030710;border:1px solid #1a2a45;border-radius:10px;padding:14px;">
            <div style="font-family:'Share Tech Mono',monospace;font-size:14px;color:#f0b429;margin-bottom:8px;">RSI PERIOD (14)</div>
          ...
          <div style="background:#030710;border:1px solid #1a2a45;border-radius:10px;padding:14px;">
            <div style="font-family:'Share Tech Mono',monospace;font-size:14px;color:#f0b429;margin-bottom:8px;">SMOOTHING FACTOR (5)</div>
          ...
          <div style="background:#030710;border:1px solid #1a2a45;border-radius:10px;padding:14px;">
            <div style="font-family:'Share Tech Mono',monospace;font-size:14px;color:#f0b429;margin-bottom:8px;">QQE FACTOR (4.238)</div>
          ...
          <div style="background:#030710;border:1px solid #1a2a45;border-radius:10px;padding:14px;">
            <div style="font-family:'Share Tech Mono',monospace;font-size:14px;color:#f0b429;margin-bottom:8px;">RSI THRESHOLD (3.0)</div>
    ...
    with edu_t[1]:
        st.markdown("""<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
          <div style="background:linear-gradient(135deg,#001508,#0a1020);border:1px solid #22c55e;border-radius:12px;padding:18px;">
            <div style="font-family:'Share Tech Mono',monospace;font-size:15px;color:#22c55e;margin-bottom:10px;">SEÑAL CALL (BUY)</div>
          ...
          <div style="background:linear-gradient(135deg,#150000,#0a1020);border:1px solid #ef4444;border-radius:12px;padding:18px;">
            <div style="font-family:'Share Tech Mono',monospace;font-size:15px;color:#ef4444;margin-bottom:10px;">SEÑAL PUT (SELL)</div>
            <div style="font-size:13px;color:#c8d8e8;line-height:1.8;">
    ...
    with edu_t[2]:
        st.markdown("""<div style="background:#0a1020;border:1px solid #1a2a45;border-left:4px solid #f0b429;border-radius:12px;padding:18px;">
          <div style="font-family:'Share Tech Mono',monospace;font-size:15px;color:#f0b429;margin-bottom:12px;">GESTIÓN DE CAPITAL</div>
          <div style="font-size:13px;line-height:1.9;color:#c8d8e8;">
            ...
            <code style="background:#030710;padding:4px 10px;border-radius:4px;color:#60a5fa;font-size:15px;">
    ...
    with edu_t[3]:
        st.markdown("""<div style="background:#0a1020;border:1px solid #1a2a45;border-left:4px solid #60a5fa;border-radius:12px;padding:18px;">
          <div style="font-family:'Share Tech Mono',monospace;font-size:15px;color:#60a5fa;margin-bottom:12px;">PSICOLOGÍA DEL TRADING</div>
          <div style="font-size:13px;line-height:1.9;color:#c8d8e8;">
    ...
    with edu_t[4]:
        st.markdown("""<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
          <div style="background:#030710;border-left:3px solid #22c55e;border-radius:8px;padding:14px;">
            ...
            <div style="font-size:13px;color:#c8d8e8;margin-top:6px;">Mayor liquidez EUR/GBP. Movimientos limpios.<br><span style="color:#a0c8e8;">Mejor: EUR/USD, GBP/USD</span></div>
          </div>
          <div style="background:#030710;border-left:3px solid #22c55e;border-radius:8px;padding:14px;">
            <b style="color:#22c55e;">NUEVA YORK 10:00–13:00 ARG</b><br>
            <div style="font-size:13px;color:#c8d8e8;margin-top:6px;">Alta volatilidad. Solapamiento con Londres.<br><span style="color:#a0c8e8;">Mejor: todos los pares mayores</span></div>
          </div>
          <div style="background:#030710;border-left:3px solid #f0b429;border-radius:8px;padding:14px;">
            ...
            <div style="font-size:13px;color:#c8d8e8;margin-top:6px;">Máxima actividad y spreads bajos.<br><span style="color:#a0c8e8;">Optimo para scalping</span></div>
          </div>
          <div style="background:#030710;border-left:3px solid #ef4444;border-radius:8px;padding:14px;">
            ...
            <div style="font-size:13px;color:#c8d8e8;margin-top:6px;">Baja volatilidad. Muchas señales falsas.<br><span style="color:#a0c8e8;">EVITAR operaciones</span></div>
          </div>
        </div>""", unsafe_allow_html=True)
...
with tab_mejoras:
            ...
            if btn_m1:
                ...
                with st.spinner("Consultando 3 timeframes..."):
                    ...
                    st.markdown(f"""<div style="background:{_mtf_bg};border:2px solid {_mtf_col};border-radius:12px;padding:16px;margin-bottom:12px;text-align:center;">
                      ...
                      <div style="font-family:'Share Tech Mono',monospace;font-size:15px;color:#a0c8e8;margin-top:6px;">{m1_act} · Confluencia Multi-Timeframe</div>
                    </div>""", unsafe_allow_html=True)
        ...
        with m2c2:
            if btn_m2:
                ...
                if _d_m2_raw is not None and len(_d_m2_raw) >= 14:
                    ...
                    st.markdown('<div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#a0c8e8;margin:10px 0 4px;">POSICIÓN DEL ATR EN RANGO HISTÓRICO</div>', unsafe_allow_html=True)
    ...
    with mej_tabs[2]:
        ...
        st.markdown(f"""<div style="background:#0a1020;border:2px solid {_kelly_color};border-radius:12px;padding:16px;text-align:center;margin-top:12px;">
          ...
          <div style="font-family:'Share Tech Mono',monospace;font-size:15px;color:#a0c8e8;margin-top:6px;">{_kelly_txt} · Racha pérdidas: {_racha_actual} · Racha ganancias: {_racha_wins}</div>
        </div>""", unsafe_allow_html=True)
...
<i>QQE Command v11 · Goya, Corrientes</i>"""
            ...
            st.markdown(f"""<div style="background:#0a1020;border:1px solid #a78bfa;border-radius:12px;padding:16px;margin-bottom:12px;font-family:'Share Tech Mono',monospace;font-size:15px;color:#c8d8e8;line-height:1.9;white-space:pre-wrap;">{_msg_m4}</div>""", unsafe_allow_html=True)
                    ...
                    if _d6 is None or len(_d6) < 40:
                        ...
                        for _ses6, _col6 in [("LONDRÉS","#22c55e"),("SOLAPAMIENTO","#f0b429"),("NUEVA YORK","#22c55e"),("ASIA/CERRADO","#ef4444")]:
                            ...
                            st.markdown(f"""<div style="background:#0a1020;border-left:4px solid {_col6};border-radius:8px;padding:12px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;">
                              <div style="font-family:'Share Tech Mono',monospace;font-size:15px;color:{_col6};">{_ses6}</div>
                              <div style="font-family:'Rajdhani',sans-serif;font-size:24px;font-weight:700;color:{_wr6_c};">{_wr6:.1f}% WR</div>
                              <div style="font-family:'Share Tech Mono',monospace;font-size:15px;color:#a0c8e8;">{_w6v}W / {_t6v-_w6v}L de {_t6v} señales</div>
                            </div>""", unsafe_allow_html=True)
            ...
            if btn_m7:
                with st.spinner("Preparando panel operativo..."):
                    ...
                    st.markdown(f"""<div style="background:#0a1020;border:3px solid {_panel_col};border-radius:14px;padding:20px;text-align:center;margin-bottom:14px;">
                      ...
                      <div style="font-family:'Share Tech Mono',monospace;font-size:15px;color:#a0c8e8;margin-top:6px;">{_ok_count}/5 condiciones OK · {m7_act} · {_dir7} · ${_monto7:.2f} · {m7_exp}</div>
                    </div>""", unsafe_allow_html=True)
                    ...
                    for _lbl7, _ok7, _det7 in _checks:
                        ...
                        st.markdown(f"""<div style="background:#030710;border-radius:6px;padding:10px 14px;margin-bottom:6px;display:flex;justify-content:space-between;align-items:center;">
                          <div style="font-family:'Share Tech Mono',monospace;font-size:15px;color:#c8d8e8;">{_ic7} {_lbl7}</div>
                          <div style="font-family:'Share Tech Mono',monospace;font-size:15px;color:{_c7};">{_det7}</div>
                        </div>""", unsafe_allow_html=True)
...
Respondé con: 1) Por qué se generó esta señal 2) Factores que la respaldan 3) Riesgos a considerar 4) Recomendación final. Máximo 200 palabras. En español."""
    ...
    with mej_tabs[8]:
        ...
        with m9c1:
            ...
            st.markdown('<div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#fbbf24;margin:10px 0 6px;">GRILLA DE BÚSQUEDA</div>', unsafe_allow_html=True)
                ...
                if _d9 is None or len(_d9) < 50:
                    ...
                    if _resultados9:
                        ...
                        st.markdown(f"""<div style="background:linear-gradient(135deg,#0a1a00,#0a1020);border:2px solid #fbbf24;border-radius:12px;padding:16px;margin-bottom:12px;text-align:center;">
                          <div style="font-family:'Share Tech Mono',monospace;font-size:14px;color:#fbbf24;margin-bottom:6px;">MEJORES PARÁMETROS ENCONTRADOS</div>
                          ...
                          <div style="font-family:'Share Tech Mono',monospace;font-size:15px;color:#c8d8e8;margin-top:8px;">
    ...
    with mej_tabs[9]:
        ...
        with m10c1:
            st.markdown('<div style="font-family:Share Tech Mono,monospace;font-size:14px;color:#c084fc;margin-bottom:10px;letter-spacing:2px;">NUEVA ENTRADA</div>', unsafe_allow_html=True)
            ...
            if _dp:
                ...
                for _emo in _dp_df["emocion"].unique():
                    ...
                    st.markdown(f"""<div style="background:#030710;border-radius:6px;padding:10px 14px;margin-bottom:6px;display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;">
                      ...
                      <div style="font-family:'Share Tech Mono',monospace;font-size:15px;color:#a0c8e8;">{len(_emo_df)} trades</div>
                    </div>""", unsafe_allow_html=True)
                    _progbar(_emo_wr, _emo_col)

                # Tabla completa
                with st.expander("📋 HISTORIAL COMPLETO DIARIO+", expanded=False):
                ...
                st.markdown("""<div style="text-align:center;padding:40px;background:#0a1020;border:2px dashed #1a2a45;border-radius:12px;">
                  ...
                  <div style="font-family:'Rajdhani',sans-serif;font-size:20px;color:#a0c8e8;">Registrá tu primera operación con estado emocional</div>
                </div>""", unsafe_allow_html=True)
...
<div style="text-align:center;margin-top:24px;padding:14px;border-top:1px solid #1e3050;">
  <div style="font-family:Share Tech Mono,monospace;font-size:15px;color:#7090b0;letter-spacing:2px;">
